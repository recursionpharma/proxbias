...
def plot_heatmap(
    ...
    if show_chr_lines or show_chroms or show_cent_lines or show_chrom_arms:
        ...
        chr_mids = pd.DataFrame((np.insert(chr_pos.values[:-1], 0, 0) +
                                chr_pos.values)/2, index=chr_pos.index).to_dict()[0]
    ...
    if show_chroms:
        ...
        for ch in chr_mids:
            ...
            if label_locy == None:
                label_locy = -0.008*s
            # Labels on top
            ax.text(chr_mids[ch], label_locy, ch.replace('chr', ''),
                    ha='center', va='bottom', rotation=lab_rot, size=lab_s)
            # Labels on the right
