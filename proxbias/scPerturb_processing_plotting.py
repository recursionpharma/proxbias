...
def _compute_chromosomal_loss(
    ...
    """
    Compute chromosomal loss in both the 3' and 5' regions of the cut site for all gene perturbations in the provided AnnData object.
    The passed AnnData object is expected to have CNV values available as generated by the infercnvpy library.
    Identities of the cells with loss are also recorded in the result DataFrame.

    Args:
        anndat (AnnData): AnnData object containing the data with the CNV values.
        blocksize (int): Block size that was used for computing the CNV values by the infercnv call.
            Needed to identify the number of blocks to consider for the gene neighborhood specified in the neighborhood_cnt parameter.
        neighborhood_cnt (int): Number of neighboring genes to consider. Default is 150.
        frac_cutoff (float): Cutoff fraction for low CNV. Default is 0.7, which means we expect 70% or more of the genes in the
            neighborhood to have low CNV.
...
def generate_specific_loss_and_summary_tables(
    ...
    summaryres_df.to_csv(get_specific_loss_summary_file_path(), index=False)
...
def plot_loss_for_selected_genes(
    ...
    Note:
        ...
        If the `chromosome_info` argument is not provided, it calls the `_get_chromosome_info` function to obtain the chromosome information.
        For each filename in the list, it reads the corresponding h5ad file, extracts relevant information from the "allres" DataFrame,
        and performs infercnv analysis on the data for the genes with specific loss passing the affected cell count threshold.
